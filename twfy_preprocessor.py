...
# Set folder paths for XML input, JSON output, and merged intermediate XML files
_XML_INPUT_FOLDER = ".\\TWFYScraper\\xml\\"
_JSON_OUTPUT_FOLDER = ".\\TWFYPreprocessor\\json\\"
_MERGED_XML_FOLDER = ".\\TWFYPreprocessor\\merged\\"

# Check whether the input and output folders exist
os.makedirs(_XML_INPUT_FOLDER, exist_ok=True)
os.makedirs(_JSON_OUTPUT_FOLDER, exist_ok=True)
os.makedirs(_MERGED_XML_FOLDER, exist_ok=True)
...
def match_file_dates():
    ...
    merge_list = []
    for xml_file in os.listdir(_XML_INPUT_FOLDER):
        match = re.search(_DATE_PATTERN, xml_file)
        matching_files = [xml_file for xml_file in os.listdir(_XML_INPUT_FOLDER) if match.group() in xml_file]
...
def merge_xml_data(file_list):
    ...
    for files in file_list:
        ...
        for xml_file in files:
            xml_path = os.path.join(_XML_INPUT_FOLDER, xml_file)
